# 100 Days of WordPress Development

Documenting my journey learning WordPress Development, Theme Development, Plugin Development, Gutenberg, and WordPress Core Contribution.

By the way my WordPress journey started year ago, but i thought do record my upcoming journey that can help people who are new to WordPress.

## Progress

- [x] Day 1 - Template Hierarchy
- [x] Day 2 - Loop and WP_Query
- [x] Day 3 - Custom Post Types
- [x] Day 4 - Custom Taxonomies
- [x] Day 5 - Custom Fields
- [x] Day 6 - Theme Customizer
- [x] Day 7 - Plugin Structure
- [x] Day 8 - Plugin Hooks
- [x] Day 9 - Shortcodes
- [x] Day 10 - Admin Notices
- [x] Day 11 - WordPress REST API
- [x] Day 12 - Sanitization and Escaping
- [x] Day 13 - Options and Settings API
- [x] Day 14 - Database Structure and Custom Tables
- [x] Day 15 - Post Meta and Custom Meta Boxes
- [x] Day 16 - WordPress AJAX
- [x] Day 17 - Roles and Capabilities
- [x] Day 18 - WP-Cron and Scheduled Tasks
- [x] Day 19 - Transients API and Caching
- [x] Day 20 - REST API Authentication
- [x] Day 21 - Gutenberg Block Development Part 1
- [x] Day 22 - Gutenberg Block Development Part 2
- [x] Day 23 - Gutenberg Block Development Part 3
- [x] Day 24 - Dynamic Gutenberg Blocks
- [x] Day 25 - Dynamic Property Card + Custom Post Type
- [x] Day 26 - WordPress Security
- [x] Day 27 - Actions & Filters 
- [x] Day 28 - WordPress Plugin Development

## Goals

- Build custom themes
- Build Gutenberg blocks
- Contribute to WordPress Core
- Learn WPGraphQL
- Learn Headless WordPress